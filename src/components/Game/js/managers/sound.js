...
class SoundManager {
  ...
  unmute() {
    this.sounds.forEach((sound) => {
      const original = sounds.find(
        (originalSound) => originalSound.name === sound.name,
      )
   ...
   */
  _loadSound(sound) {
    let loadedSound = new window.BABYLON.Sound(
      sound.name,
      sound.file,
      this._scene,
      null,
    ...
    if (sound.follow)
      loadedSound.attachToMesh(nodeRenderer.getPlayer().getMesh())
    return loadedSound
  }
}
